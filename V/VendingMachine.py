...
def get_more(snacks, coins_available):
    notification_sound = pygame.mixer.Sound("AlertAudio.wav")
...
class VendingMachine(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.purchased_sound = pygame.mixer.Sound("PurchasedSound.wav")
        ...
        # Create and populate a Combobox for snacks
        self.snack_box = ttk.Combobox(self, textvariable=self.snack_var, values=list(snacks.keys()))
        self.snack_box.grid(row=0, column=1)

        tk.Label(self, text="Enter payment: ").grid(row=1, column=0)
        
        # You can also create a Combobox for payment methods, or just keep it as a text Entry
        self.payment_entry = tk.Entry(self, textvariable=self.payment_var, width=50)
        self.payment_entry.grid(row=1, column=1)
    ...
    def pay(self):
        ...
        if snack not in snacks or snacks[snack][1] <= 0:
            messagebox.showinfo("Error", f"We don't have {snack} as a snack or its out of stock, please select again.")
            return
    ...
    def shortest_amount_of_coins(self, coins, coins_available, return_amount):
        ...
        if return_amount != 0:
            messagebox.showinfo("Unable to make exact change with available coins")
            return None
        return result
